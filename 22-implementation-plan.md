...
| **1** | `shared` | shared→25 |
| **2** | `identity-access` · `quota` · `accounting-ledger` · `exchange-rate` · `reporting` | identity-access→16, quota→1 |
| **3** | `network` · `promotions` · `storage` | network→7, promotions→2, storage→1 |
| **4** | `documents` | documents→5 |
| **5** | `tenancy` · `fleet` · `customer` | tenancy→7, fleet→4, customer→1 |
| **6** | `agent` · `staff` | agent→6, staff→1 |
| **7** | `scheduling` · `wallet-ledger` | scheduling→6, wallet-ledger→3 |
| **8** | `fare` | fare→3 |
| **9** | `seat-inventory` | seat-inventory→2 |
| **10** | `booking` | booking→4 |
| **11** | `payment-settlement` · `charter` | payment-settlement→3 |
| **12** | `notification` · `payment-gateway-adapter` | notification→2 |
| **13** | `fiscal-adapter` · `uts-adapter` | — |
...
WAVE 0 ─────────────────────── api-contracts ⭐26      pure types, no dependency at all
                                      │
                                      ▼
WAVE 1 ───────────────────────── shared ⭐25           machinery, and it may refuse
                                      │
            ┌─────────────────────────┼───────────────────────────┐
            ▼                         ▼                           ▼
WAVE 2 ── identity-access ⭐16   quota   exchange-rate   reporting   accounting-ledger
                         │        │     └──── block nothing; build any time ────┘
            ┌────────────┼────────────┐
            ▼            ▼            ▼
WAVE 3 ── network ⭐7  promotions   storage
                                      │
                                      ▼
WAVE 4 ──────────────────────── documents ⭐5        ← the chokepoint
            ...
            ┌─────────────────────────┼───────────┐
            ▼                         ▼           ▼
WAVE 5 ── tenancy ⭐7               fleet       customer
            │                         │
            ├───────────┐             │
            ▼           ▼             │
WAVE 6 ── agent ⭐6    staff          │
            │           │             │
            ▼           ▼             │
WAVE 7 ── wallet-ledger  scheduling ⭐6 ◄┘
                         │
                         ▼
WAVE 8 ───────────────  fare
                         │
...
WAVE 9 ────────────  seat-inventory
                         │
                         ▼
WAVE 10 ─────────────  booking ⭐4      ← also needs quota, promotions, customer, network, tenancy
                         │
            ┌────────────┴────────────┐
            ▼                         ▼
WAVE 11 ─ payment-settlement       charter
            │
            ├─────────────────────────┐
            ▼                         ▼
WAVE 12 ─ notification        payment-gateway-adapter
            │
            ├─────────────────────────┐
            ▼                         ▼
WAVE 13 ─ fiscal-adapter          uts-adapter
```
...
```
api-contracts → shared → identity-access → storage → documents → tenancy → staff → scheduling → fare → seat-inventory → booking → payment-settlement → notification → uts-adapter
```

**14 of 27 modules sit on it.** Every step is a hard sequence; nothing shortens it, and slipping any one of
...
|---|---|---|---:|
| **1 · Foundation** | 0–4 | Somebody can log in. Files can be stored and typed. | 11 |
...
| **4 · Money and externals** | 11–13 | Settlement, notification, gateway, regulator, charter. | 6 |
